...
// @desc    verify Coupon
// @route   POST /api/coupon/verify/:id
// @access  Public
const verifyCoupon = async (req, res) => {
  try {
    const _id = req.params.id;
    
    const coupon = await Coupon.findOne({ couponCode: _id });
    if (coupon) {
      if (coupon.valid) {
        console.log(coupon);
        if (coupon.couponUsed <= coupon.limit) {
          const { _id, couponCode, discount } = coupon;
...
const updateCoupon = async (req, res) => {
  const _id = req.params.id;
  const { couponCode, discount, limit, valid } = req.body;
  try {
